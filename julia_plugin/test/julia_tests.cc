...
TEST_CASE("Tensors of different dtypes") {
    
    auto model = std::make_shared<phasm::JuliaModel>("TypedModel.jl");

    phasm::Surrogate f_surrogate = phasm::SurrogateBuilder()
        .set_model(model)
        .local_primitive<float>("mat", phasm::INOUT, {2, 3})
        .local_primitive<int16_t>("sum", phasm::OUT)
        .finish();

    float matrix[6] = { 2.0, 3.0, 7.0, 14.0, 9.0, 1.0};  // This is actually a 2x3 matrix as far as PHASM and Julia are concerned
    int16_t sum = 0;
    // std::cout << "Before call_model(): x = " << x << ", f = " << f << std::endl;
