...
void EventAction::EndOfEventAction(const G4Event* event)
{
  ...
  auto gapTrackLength
    ...
    auto primary_particle = event->GetPrimaryVertex(0)->GetPrimary();
    particle_momentum = primary_particle->GetTotalMomentum();
    particle_pz = primary_particle->GetPz();
    auto zdir = primary_particle->GetMomentumDirection().z();
    auto theta = acos(zdir) / CLHEP::deg;
  ...
  analysisManager->FillH2(0,particle_momentum,gapEdep);
    analysisManager->FillH2(1,theta,gapEdep);
  ...
  analysisManager->FillNtupleDColumn(4,particle_momentum);
  analysisManager->FillNtupleDColumn(5,particle_pz);
    analysisManager->FillNtupleDColumn(6,zdir);
